...
def series_colours(c):
    """Four distinguishable values for categorical series.

    The ink/vermilion pairing carries a specific meaning elsewhere in the
    archive — within the standard versus breaching it. These charts compare
    four specialties instead, so the same values are reused in a categorical
    register. No new hues are introduced.
    """
    return [c["ink"], c["teal"], c["vermilion"], c["muted"]]
